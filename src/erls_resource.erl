...
request(State, Method, Path, Headers, Params, Body, Options) ->
    ...
    io:fwrite("erls_resource ~p:request/7:Path1=~p~n",[?LINE, Path1]),
...
do_request(#erls_params{host=Host, port=Port, ssl=Ssl, timeout=Timeout},
    ...
    io:fwrite("erls_resource ~p:do_request/7:InitialBody=~p~n",[?LINE, InitialBody]),
                ...
                true ->
                    case is_pid(Body) of
                        true ->
                            {ok, Body};
                        false ->
                            try jsonerl:decode(binary_to_list(Body)) of
                                Resp1 ->
                                    case Resp1 of
                                        {[{<<"ok">>, true}]} -> ok;
                                        {[{<<"ok">>, true}|Res]} -> {ok, {Res}};
