...
find({Params, Index}, Id) when is_list(Id) ->
    ...
    Res = erls_resource:get(Params, 
                            filename:join([Index, Collection, ESId]), 
                            [], [], []),
    case Res of
        {ok, Json} ->
...
find({Params, Index}, Type, [{Key, 'equals', Value}], Max, Skip, Sort, SortOrder) 
  when is_atom(Type), 
    ...
    case boss_record_lib:ensure_loaded(Type) of
        true ->
            ...
            Res = erlastic_search:search([Index], [Collection], Value),
            case Res of
                {ok, Json} ->
...
delete({Params, Index}, Id) when is_list(Id) ->
    ...
    Res = erlastic_search:delete_doc(Params, Index, Collection, Id),
    case Res of
        {ok, _} ->
            ok;
        _ ->
    ...
    Res = 
        case Record:id() of
            id ->
                ...
                erlastic_search:index_doc(Params, Index, Collection, Doc);
            DefinedId when is_list(DefinedId) ->
                ...
                erlastic_search:index_doc_with_id(Params, Index, Collection, PackedId, Doc)
           end,
    case Res of
        {ok, Json} ->
